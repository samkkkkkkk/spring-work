...
public class UserLoginHandler implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		System.out.println("나는 preHanle 이다!!!");
		return HandlerInterceptor.super.preHandle(request, response, handler);
	}
	
	//postHandle은 컨트롤러를 나갈 때 공통 처리해야 할 내용을 작성
	// /userLogin 이라는 요청 컨트롤러에서 마무리 된 후, viweResolver로 전달이 되기 전
	// 로그인 성공 or 실패 여부에 따라 처리할 로직을 작성할 계획입니다.
	@Override
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
			ModelAndView modelAndView) throws Exception {
		System.out.println("로그인 인터셉터 동작합니다!");
		System.out.println("요청 방식: " + request.getMethod()); //GET 인지 POST방식인지 알아보는 과정
		...
		if(request.getMethod().equals("POST")) { //요청 방식이 POST라면 -> 로그인 요청이었다면.
			...
			if(result != null) {//로그인 성공 
				...
				//로그인 성공한 회원에게는 세션 데이터를 생성해서 로그이니 유지를 하게 해 줌.

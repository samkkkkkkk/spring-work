...
<body>
	...
	<script>
		...
		function regist(){
			...
			//세션에서 현재 로그인 중인 사용자의 정보('id')를 얻어오자
			const userId = '${sessionScope.login}';
			if(userId===''){//세션 데이터가 없다. 로그인을 안했다.
				alert('로그인이 필요한 서비스 입니다.')
				return;
			}

			//자바스크리브로 첨부한 파일 확장자 체크.
			let file = document.getElementById('file').value;
			console.log(file);
			//.을 제거한후 확장자만 얻어내어 소문자로 일괄 변경.
			file =file.slice(file.indexOf('.')+1).toLowerCase();

			if(file !== 'jpg' && file !=='png' &&file!=='jpeg' && file!=='bmp'){
				alert('이미지 파일만 첨부할 수 있습니다. (jpg,png,jpeg,bmp)')
				return;
			}
			/*
			비동기 방식 요청에서 from을 생성해서 보내주는 방법

			->FormData 객체를 활용하면 됨.
			무조건 이 방식이 옳은것은 아니다. FormData는 비동기 방식이 꼭 필요한 경우에만 사용함.
			대부분의 경우엔 form 태그를 이용해서 전송하는 방식이 더 간편하고 더 자주 사용된다.
			*/
	...
	$contentDiv.addEventListener('click',e=>{
			...
			.then(data => {
				...
				document.querySelector('.modal-img img').src = '${pageContext.request.contextPath}/snsboard/display/' + data.fileLoca + '/' + data.fileName;
				document.getElementById('snsWriter').innerHTML = data.writer
				$('#snsModal').modal('show');

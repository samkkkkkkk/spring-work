...
@Controller
@RequestMapping("/request") //컨트롤러 자체의 공통된 URI 맵핑
public class RequestController {
	
	public RequestController() {
		System.out.println("RequestController가 생성됨!");
	}
...
//	@RequestMapping(value="/request/basic01", method = RequestMethod.POST)
	...
	/*
	 ...
	 1. 전통적인 jsp/servlet 방식의 파라미터 읽기 처리 방식
	 - HttpServletRequest request 객체를 활용 (우리가 jsp에서 사용하던 방식)
	 -> 스프링에서는 자 사용하지 않음
	...
	*/
	...
	/*
	 ...
	 - 파라미터 데이터와 연동되는 VO 크래스가 필요합니다.
	 - VO 클래스의 필드는 파라미터 변수명과 동일하게 작성합니다. (setter 메서드 호출)
	 
	 # 커멘드 객체: 사용자의 입력을 담기 위해 설계되고, 특정 검증 로직이나 비지니스 로직을
